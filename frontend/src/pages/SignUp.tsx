...
import { Sprout, Mail, Lock, User, Phone, Eye, EyeOff, ArrowRight, Loader2 } from 'lucide-react'

interface SignUpProps {
  onSignUp: (fullName: string, email: string, password: string, phone?: string) => Promise<any>
  onGoogleLogin: () => Promise<any>
  onSwitchToSignIn: () => void
}

export function SignUp({ onSignUp, onGoogleLogin, onSwitchToSignIn }: SignUpProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      await onSignUp(fullName, email, password, phone || undefined)
    } catch (err: any) {
    ...
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #0a1f0a 0%, #132a13 25%, #1a3a1a 50%, #0d2818 75%, #071a0f 100%)' }}>
      ...
      <div className="absolute inset-0 overflow-hidden">
        ...
        <motion.div
          ...
          className="absolute -bottom-60 -left-40 w-[500px] h-[500px] rounded-full opacity-8"
      ...
      >
        ...
        <div className="backdrop-blur-xl bg-white/10 rounded-3xl border border-white/20 shadow-2xl p-8">
          ...
          >
            ...
            <p className="text-sm text-white/60 mt-1">Join AgroNexus — Smart Cold Chain Monitoring</p>
          </motion.div>
          ...
          <div className="flex items-center gap-4 my-6">
            ...
            <span className="text-xs text-white/40 font-medium">or sign up with email</span>
            <div className="flex-1 h-px bg-white/20"/>
          </div>
